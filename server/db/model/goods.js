...
exports.insertGoods = (ctx) => __awaiter(this, void 0, void 0, function* () {
    ...
    yield mysql.execute(sql, [null, title, subTitle, mainImg, thumbnail, images, state, price, salePrice, stock, tag, 0, date, date, categoryId, companyId]);
});
//修改一个商品
exports.updateGoods = (ctx) => __awaiter(this, void 0, void 0, function* () {
    ...
    let date = Date.now();
    yield mysql.execute(sql, [title, subTitle, state, mainImg, thumbnail, images, price, salePrice, stock, tag, date, categoryId, goodsId]);
});
//删除一个商品
...
//获取菜品通用备注
exports.getTags = (ctx) => __awaiter(this, void 0, void 0, function* () {
    let mysql = ctx.db;
    let { companyId } = ctx.query;
...
//新增或修改菜品通用备注
exports.updateTags = (ctx) => __awaiter(this, void 0, void 0, function* () {
    let mysql = ctx.db;
    let { id, tag, companyId } = ctx.params;
    if (id) {
        let sql = `update tags set tag = ? where companyId = ?`;
        yield mysql.execute(sql, [tag, companyId]);
    }
    else {
        let sql = `insert into tags(id, tag, companyId) values(?, ?, ?)`;
        yield mysql.execute(sql, [null, tag, companyId]);
    }
});
